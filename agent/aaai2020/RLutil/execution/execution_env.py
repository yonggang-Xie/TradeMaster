...
from agent.aaai2020.RLutil.execution.utils.feature_type import FeatureType
...
class ExecutionEnv_TwoActions(gym.Env):
    ...
    def configure_(self,
        ...
        if self.featureType == FeatureType.ORDERS:
            self.observation_space = spaces.Box(low=0.0,
                                                high=10.0,
                                                shape=(2 * self.lookback + 1,
                                                       self.bookSize, 2))
        else:
            self.observation_space = spaces.Box(low=0.0,
                                                high=100.0,
                                                shape=(self.lookback + 1, 3))
    ...
    def create_execution_(self, a, inventory):
        ...
        if runtime <= 0.0 or a is None:
            ...
            if a == 'skip':
                ...
                currentOrderInventory = int(inventory *
                                            self.totalOrderInventory /
                                            self.inventoryLevels[-1])
    ...
    def update_execution_(self, execution, a, inventory):
        ...
        if runtime <= 0.0 or a is None:
            ...
            if a == 'skip':
                currentOrderInventory = 0
                price = execution.getOrderbookState().getPriceAtLevel(
                    self.side, 0)
            else:
                # currentOrderInventory = self.actionState.getI()
                # currentOrderInventory = min(self.actionState.getI(), self.totalOrderInventory/10)
                currentOrderInventory = min(
                    self.actionState.getI(),
                    int(inventory * self.totalOrderInventory /
                        self.inventoryLevels[-1]))
                price = execution.getOrderbookState().getPriceAtLevel(
                    self.side, a)
            ot = OrderType.LIMIT
    ...
    def step_(self, action, inventory):
        ...
        state_next = ActionState(t_next, i_next,
                                 self.execution.getReferencePrice(),
    ...
    def reset_(self, t, i):
        ...
        state = ActionState(
            t, i, orderbookState.getBestAsk(),
            {self.featureType.value: feature})  # np.array([[t, i]])
        self.execution = None
    ...
    def render(self, mode='human', close=False):
        ...
        for i in range(-40, 80):
            lastOrderbookState = self.orderbook.getState(
                self.initOrderbookIndex + i)
            list_of_datetimes.append(lastOrderbookState.timestamp)
            values.append(lastOrderbookState.tradePrice)
        difference = [(list_of_datetimes[i + 1] - date).total_seconds()
                      for (i, date) in enumerate(list_of_datetimes)
                      if i < len(list_of_datetimes) - 1]

        if self.done:
            trades = self.execution.getTrades()
            reward = self.execution.calculateReward(trades)
            direction = "Buy" if trades[
                0].orderSide == OrderSide.BUY else "Sell"
            delta_tradeTime = [
                (trade.timestamp - list_of_datetimes[0]).total_seconds()
                for trade in trades
            ]
            trade_x = []
            for delta in delta_tradeTime:
                for i in range(len(difference)):
                    if delta == sum(difference[:i]):
                        trade_x.append(i + 1)
            tradeTime = [
                trade.timestamp.strftime("%H:%M:%S") for trade in trades
            ]
            tradePrice = [trade.price for trade in trades]
            text = [[int(trade.cty), '{0:.2f}'.format(trade.price)]
                    for trade in trades]
            ...
            segments_y = np.r_[values[0], values[1:-1].repeat(2),
                               values[-1]].reshape(-1, 2)

            linecolors = [
                'c' if x_[1] < 40 or x_[0] >= 100 else 'blue'
                for x_ in segments_x
            ]
            plt.gca().add_collection(
                LineCollection(segments, colors=linecolors))
            plt.xlim(x.min(), x.max())
            plt.ylim(values.min() - 0.05, values.max() + 0.05)

            frequency = 20
            test_dates = [
                date.strftime("%H:%M:%S") for date in list_of_datetimes
            ]
            plt.xticks(list(range(0, x_len, frequency)),
                       test_dates[:x_len:frequency],
                       rotation=20)
