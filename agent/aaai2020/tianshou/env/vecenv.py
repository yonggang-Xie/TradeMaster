...
class StockVectorEnv(BaseVectorEnv):
    """docstring for VectorEnv"""

    def __init__(self, env_fns, mode='train'):
        super().__init__(env_fns)
        self.envs = [_() for _ in env_fns]
        self.t_indexs = [0 for _ in self.envs]
        self.i_indexs = [1 for _ in self.envs]
        self.mode = mode
            ...
            if id is None:
                ...
                for index, e in enumerate(self.envs):
                    ...
                    if self.i_indexs[index] < len(e.I) - 1:
                        ...
                        self.i_indexs[index] = 1
                        if self.t_indexs[index] < len(e.T)-1:
                            self.t_indexs[index] = self.t_indexs[index] + 1
                        else:
                            self.t_indexs[index] = 0
...
class StockVectorEnv_TwoActions(BaseVectorEnv):
    """docstring for VectorEnv"""
